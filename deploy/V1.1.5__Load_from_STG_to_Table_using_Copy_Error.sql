...
copy into ch01.customer_csv_error (
    customer_pk,
    salutation,
    first_name,
    last_name,
...
from (
        ...
        from @ttips.ch01.csv_stg/data-file-with-2-issues.csv  t
    ) file_format = (format_name = 'ttips.ch01.customer_csv_ff') on_error = ABORT_STATEMENT;
...
from (
        select t.$1::text as customer_pk,
            ...
            metadata$file_content_key as stg_file_md5,
            current_timestamp as copy_data_ts
        from @ttips.ch01.csv_stg/data-file-with-datatype-issue.csv  t
    ) file_format = (format_name = 'ttips.ch01.customer_csv_ff') on_error = ABORT_STATEMENT;
